...
import { BookOpen, UserCircle, Info, Edit2, Trash2, Trash } from "lucide-react";
...
const BookCard = ({ book, setBooks }) => {
  ...
  return (
    <>
    {/* CARD */}
    <Link to={`/book/${book._id}` } className={`realative block rounded-xl bg-base-100 p-4 border transition-all duration-200${isActive ? "border-primary shadow-lg": "border-base-300"} hover:border-primary hover:shadow-xl`} >
    
    {/* Top Row */}
    <div className="flex justify-between items-start">
      <p className="text-xs text-base-conetent/60 truncate"> {book._id} </p>
